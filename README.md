...
**Reading the one thing that is not a guess.** The first briefing this system ever sent said *"WHY THEY BOOKED — unknown. Do not guess on the call, ask."* — honest, and the honesty was only necessary because nothing read the part of the mail where the answer lives. cal.com renders the free-text **Additional notes** field, and renders **each custom booking question as its own heading**, into the same confirmation email; those words existed in the parser only as `_LABEL_WORDS` — labels to avoid mistaking for a person's name — and nothing ever read what sat *underneath* them. So `notes` is now parsed, stored, and quoted at the very top of the briefing, above every inference the module makes, labelled as theirs. When a note exists the briefing stops telling you to ask why (a briefing that asks a question the person already answered reads as one nobody read) and the 15-minute plan opens with *"I read your note — tell me more about that"* instead. Two traps are pinned by tests: cal.com's own footer heading is phrased as a question (*"Need to make a change?"* → `Reschedule Cancel` is not why a stranger booked a call), and a note's own closing question (*"…Can you review it?"*) was being mistaken for the next question-heading and cutting the note in half. What separates them is what comes next — a heading is followed by its answer, a closing sentence by the next section. The note is **stored but never printed by `calls --list`**: that command runs in a public Actions log, and these are a stranger's own words about their business, so the log gets *"note: yes, 142 chars — in the briefing email"* and the private email gets the words.
...
**The channel the numbers actually favour — and the metric that could not see it.** In one month the outbound half sent **23 cold emails → 1 reply → 0 calls**. In the same month **one LinkedIn post produced a booked call the next morning**. That comparison was invisible for two reasons, both now fixed. First, `kpi.funnel` counted `calls_booked` from `OutreachRecord.call_booked_at` — a field stamped *only* when the invitee is someone we cold-emailed — so the only real booking this system has ever produced read as `calls_booked: 0` in every report while a briefing about it sat in the owner's inbox. A metric that cannot rise for the channel that converts is not a metric. It now counts `CallRecord` too, split `from outreach` / `inbound`, deduped on the address because an outreach booking is normally evidenced twice (the stamp *and* the row the inbox sweep wrote) and summing would double it. `booking_rate_pct` deliberately stays an **outreach** rate: an inbound booking has no reply behind it, and crediting the reply handler with calls it never touched would make the number that judges it unfalsifiable in the one direction nobody wants. Second, `verdict()` walked a ladder that assumed cold email was the only channel, so it answered *"volume is sufficient to conclude the targeting or the pitch is off"* to a week in which a call had been booked — ranking a channel with a result below a channel with none, which is how a month gets spent tuning the losing half. It now names the converting channel first, with `posts_published` as inbound's denominator. On the same evidence LinkedIn posting went from Mon/Wed/Fri to **weekdays**; if calls-per-post falls, it comes back down.
